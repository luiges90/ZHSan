    ...
    class EventTab : BaseTab<Event>
    {
        ...
        protected override Dictionary<String, String> GetHelpText()
        {
            return new Dictionary<string, string>()
            {
                ...
                { "HasHappened", "已发生过" },
                { "IsRepeatable","可以重复" },
                { "IsMinor","不重要。不重要的事件不会出现对话，除非涉及君主" },
                { "PredecessorEventID","某事件发生之后。需要在某事件发生过之后才能触发" },
                { "ProbabilityBase","发动几率。实际机率为1/此数" },
                { "GloballyDisplayed","全势力可见" },
                { "StartYear","开始年" },
                { "StartMonth","开始月" },
                { "EndYear","结束年" },
                { "EndMonth","结束月" },
                { "TriggeringCharString","武将编号。指定可能触发的武将ID，以空格分隔，先指定第k个武将，后跟随一个武将ID，如0 100 0 234 1 346 2 -1。可在同一个k指定多个武将，则代表列表中任何一个。-1代表任何武将。" },
                { "CharConditionString","武将条件。触发的武将需符合的条件，以空格分隔，先指定第k个武将，后跟随一个武将ID" },
